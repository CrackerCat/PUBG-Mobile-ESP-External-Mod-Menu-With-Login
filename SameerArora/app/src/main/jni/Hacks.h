...
void DrawESP(ESP esp, int screenWidth, int screenHeight)
{
    ...
    if(response.Success) {
        ...
        for (int i = 0; i < response.PlayerCount; i++) {
            ...
            if (response.Players[i].HeadLocation.z != 1) {
                ...
                if (x > -50 && x < screenWidth + 50) {//onScreen
                    ...
                    if (isSkelton && response.Players[i].Bone.isBone &&
                        ...
                        esp.DrawLine(Color().White(), 3, Vec2(x, y),
                                     Vec2(response.Players[i].Bone.neck.x,
                                          response.Players[i].Bone.neck.y));
                        esp.DrawLine(Color().White(), 3, Vec2(response.Players[i].Bone.neck.x,
                                                              response.Players[i].Bone.neck.y),
                                     Vec2(response.Players[i].Bone.cheast.x,
                                          response.Players[i].Bone.cheast.y));
                        esp.DrawLine(Color().White(), 3, Vec2(response.Players[i].Bone.cheast.x,
                                                              response.Players[i].Bone.cheast.y),
                        ...
                        esp.DrawLine(Color().White(), 3, Vec2(response.Players[i].Bone.lSh.x,
                                                              response.Players[i].Bone.lSh.y),
                                     Vec2(response.Players[i].Bone.lElb.x,
                                          response.Players[i].Bone.lElb.y));
                        esp.DrawLine(Color().White(), 3, Vec2(response.Players[i].Bone.rSh.x,
                                                              response.Players[i].Bone.rSh.y),
                                     Vec2(response.Players[i].Bone.rElb.x,
                                          response.Players[i].Bone.rElb.y));
                        esp.DrawLine(Color().White(), 3, Vec2(response.Players[i].Bone.lElb.x,
                                                              response.Players[i].Bone.lElb.y),
                                     Vec2(response.Players[i].Bone.lWr.x,
                                          response.Players[i].Bone.lWr.y));
                        esp.DrawLine(Color().White(), 3, Vec2(response.Players[i].Bone.rElb.x,
                                                              response.Players[i].Bone.rElb.y),
                                     Vec2(response.Players[i].Bone.rWr.x,
                                          response.Players[i].Bone.rWr.y));
                        esp.DrawLine(Color().White(), 3, Vec2(response.Players[i].Bone.pelvis.x,
                                                              response.Players[i].Bone.pelvis.y),
                        ...
                        esp.DrawLine(Color().White(), 3, Vec2(response.Players[i].Bone.lTh.x,
                                                              response.Players[i].Bone.lTh.y),
                                     Vec2(response.Players[i].Bone.lKn.x,
                                          response.Players[i].Bone.lKn.y));
                        esp.DrawLine(Color().White(), 3, Vec2(response.Players[i].Bone.rTh.x,
                                                              response.Players[i].Bone.rTh.y),
                                     Vec2(response.Players[i].Bone.rKn.x,
                                          response.Players[i].Bone.rKn.y));
                        esp.DrawLine(Color().White(), 3, Vec2(response.Players[i].Bone.lKn.x,
                                                              response.Players[i].Bone.lKn.y),
                                     Vec2(response.Players[i].Bone.lAn.x,
                                          response.Players[i].Bone.lAn.y));
                        esp.DrawLine(Color().White(), 3, Vec2(response.Players[i].Bone.rKn.x,
                                                              response.Players[i].Bone.rKn.y),
                    ...
                    if (isPlayerBox) {
                        ...
                        if (response.Players[i].Distance < 50) {
                            esp.DrawRect(clr, screenHeight / 250, Vec2(x - mx, top),
                                         Vec2(x + mx, bottom));
                        } else {
                            esp.DrawRect(nclr, screenHeight / 250, Vec2(x - mx, top),
                                         Vec2(x + mx, bottom));
                        }
                    }
            ...
            if (response.Players[i].HeadLocation.z == 1.0f) {
                ...
                if (y > screenHeight - screenHeight / 12)
                    ...
                    y = screenHeight / 12;
                if (x < screenWidth / 2) {
                    esp.DrawFilledCircle(Color(255, 0, 0,255), Vec2(screenWidth, y),
                                         screenHeight / 18);
                    sprintf(extra, "%0.0f m", response.Players[i].Distance);
                    ...
                    esp.DrawFilledCircle(Color(255, 0, 0,255), Vec2(0, y),
                                         screenHeight / 18);
                    sprintf(extra, "%0.0f m", response.Players[i].Distance);
                    esp.DrawText(Color().White(), extra,
            ...
            if(isPlayerLine) {
                ...
                if (response.Players[i].Distance < 50) {
                    esp.DrawLine(clr, screenHeight / 350,
                                 Vec2(screenWidth / 2, screenHeight / 10.5), Vec2(x, top));
                } else {
                    esp.DrawLine(nclr, screenHeight / 350,
                                 Vec2(screenWidth / 2, screenHeight / 10.5), Vec2(x, top));
                }
            }
        ...
        if (botCount + playerCount > 0) {
            ...
            if (botCount == 0){
                sprintf(extra, "BOT: %d", botCount);
                esp.DrawText(Color(30, 255, 0), extra,
                             Vec2(screenWidth / 2 + screenHeight / 12.5, screenHeight / 10.4),
                             screenHeight / 28);
            } else {
                sprintf(extra, "BOT: %d", botCount);
                esp.DrawText(Color(255, 30, 0), extra,
                             Vec2(screenWidth / 2 + screenHeight / 12.5, screenHeight / 10.4),
                             screenHeight / 28);
            }

            if (playerCount == 0){
                sprintf(extra, "ENEMY: %d", playerCount);
                esp.DrawText(Color(30, 255, 0), extra,
                             Vec2(screenWidth / 2 - screenHeight / 9.3, screenHeight / 10.4),
                             screenHeight / 28);
            } else {
                sprintf(extra, "ENEMY: %d", playerCount);
                esp.DrawText(Color(255, 30, 0), extra,
            ...
            esp.DrawLine(Color(0, 0, 0), 3, Vec2(screenWidth / 2, screenHeight / 18),
                         Vec2(screenWidth / 2, screenHeight / 10.1));
        }

        for (int i = 0; i < response.GrenadeCount; i++){
            if(!isGrenadeWarning)
                continue;
            esp.DrawText(Color(255, 255, 0),"Warning: Grenade",Vec2(screenWidth/2,screenHeight/8),textsize);
